...
export default function Home() {
  ...
  const deleteImg = async (object_key) => {
    axios
      .post(`${process.env.NEXT_PUBLIC_API_URL}/delete`, {
        file_name: object_key,
      })
      .then((res) => {
        setResponse(JSON.stringify(res.data, null, 2));
        setImages((prev) =>
          prev.filter((img) => img.object_key !== object_key),
        );
      })
        ...
        <div className="mt-2 w-full flex flex-wrap gap-3 bg-gray-200 rounded-lg p-4">
          {images.map((image, index) => (
            <div key={index} className="w-44 h-fit relative">
              <p
                className="absolute cursor-pointer text-red-500 font-bold top-1 right-2 z-20 text-lg"
                onClick={() => deleteImg(image.object_key)}
              >
                x
              </p>
              <img
                src={image.public_url}
                className="w-44 h-auto object-cover z-10"
              />
            </div>
          ))}
        </div>
